	...
	if (in_array("ALL",$admin_cat_access_array_cats)== false) {
	  ...
	  $admin_cat_access = implode(",",$admin_cat_access_array_cats);
		...
		if (MODULE_CAT_QVB_DISCOUNT_STATUS) {
			...
			$catdis = explode(',',MODULE_CAT_QVB_DISCOUNT_RATES);
			if(!empty($catdis)){
				foreach($catdis as $val){
					$piece = explode(':',$val);
					if($piece[0] !=$categories_id){
						$category_discounts[] = $val;
					}
				}
			}
			if($_POST['enable_discount'] == 1){
				$discounts_array = array();
				foreach($_POST['discount_q'] as $key => $val){
					if($val != ''){
						$discounts_array[] = $val.';'.$_POST['discount_v'][$key];
					}
				}
				$category_discounts[] = $categories_id.':'.$_POST['dc_type'].':'.$_POST['dc_method'].':'.implode(';',$discounts_array);
			}
		mysql_query("UPDATE configuration SET configuration_value='".implode(',',$category_discounts)."' WHERE configuration_key='MODULE_CAT_QVB_DISCOUNT_RATES'");
		}
...
<?php
  if ($action == 'new_product') {
	...
	// END: Extra Fields Contribution
      ...
      //Minimum quantity code
	  $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id, p.minorder from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$_GET['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
	  //End Minimum quantity code
      $product = tep_db_fetch_array($product_query);
...
<?php
    ...
    if (tep_not_null($_POST)) {
      ...
      //Minimum quantity code
	  $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.minorder from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$_GET['pID'] . "'");
	  //End: Minimum quantity code
      $product = tep_db_fetch_array($product_query);
    ...
    switch ($action) {
      ...
      case 'edit_category':
		...
		if (MODULE_CAT_QVB_DISCOUNT_STATUS == 'true') {
			$dc_types = array();
			$dc_types[] = array('id' => 'PE','text' => 'Percentage Discount');
			$dc_types[] = array('id' => 'MM','text' => 'Flat Dollar Discount');
			$dc_types[] = array('id' => 'ME','text' => 'Money off each');
			$dc_types[] = array('id' => 'IM','text' => 'Free items');
					
			$dc_methods = array();
			$dc_methods[] = array('id' => 'Q','text' => 'Based on Quantity');
			$dc_methods[] = array('id' => 'V','text' => 'Based on money value');
			...
			if(!empty($catdis)){
				foreach($catdis as $val){
					...
					if($piece[0] == $_GET['cID']){
						...
						$discounts = explode(';',$piece[3]);
					}
				}
			}
			$contents[] = array('text' => '<br><b>Enable Category Discount: </b> '.tep_draw_pull_down_menu('enable_discount', array(array('id'=>0,'text'=>'No'),array('id'=>1,'text'=>'Yes')),$hasdiscount).'<br>Discount Type: ' . tep_draw_pull_down_menu('dc_type', $dc_types,$dc_type).
											'<br>Discount Method: ' . tep_draw_pull_down_menu('dc_method', $dc_methods,$dc_method));
			for($i=0;$i<8;$i+=2){
       			 $contents[] = array('text' => '<br>' . 'Discount Quantity: ' . tep_draw_input_field('discount_q[]', $discounts[$i], 'size="4"') .' Discount Value: '.tep_draw_input_field('discount_v[]', $discounts[($i+1)], 'size="4"'));
			}
		}
