...
// EOF: KategorienAdmin / OLISWISS
          ...
          tep_mail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, sprintf(ADMIN_EMAIL_SUBJECT, STORE_NAME, $_POST['admin_firstname'], $_POST['admin_lastname']), sprintf(ADMIN_EMAIL_TEXT, $_POST['admin_firstname'], HTTP_SERVER . DIR_WS_ADMIN, $_POST['admin_email_address'], $makePassword, STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
// EOF: KategorienAdmin / OLISWISS
          ...
          tep_mail($_POST['admin_firstname'] . ' ' . $_POST['admin_lastname'], $_POST['admin_email_address'], sprintf(ADMIN_EMAIL_EDIT_SUBJECT, STORE_NAME, $_POST['admin_firstname'], $_POST['admin_lastname']), sprintf(ADMIN_EMAIL_EDIT_TEXT, $_POST['admin_firstname'], HTTP_SERVER . DIR_WS_ADMIN, $_POST['admin_email_address'], $hiddenPassword, STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
// Thomas Schittli: Start Bugfixes
  function get_admin_right_access($_my_post) {
    if ($_my_post['admin_right_access_CNEW'] == true) $admin_right_access .= "CNEW,";
    if ($_my_post['admin_right_access_CEDIT'] == true) $admin_right_access .= "CEDIT,";
    if ($_my_post['admin_right_access_CMOVE'] == true) $admin_right_access .= "CMOVE,";
    if ($_my_post['admin_right_access_CDELETE'] == true) $admin_right_access .= "CDELETE,";
    if ($_my_post['admin_right_access_PNEW'] == true) $admin_right_access .= "PNEW,";
    if ($_my_post['admin_right_access_PEDIT'] == true) $admin_right_access .= "PEDIT,";
    if ($_my_post['admin_right_access_PMOVE'] == true) $admin_right_access .= "PMOVE,";
    if ($_my_post['admin_right_access_PCOPY'] == true) $admin_right_access .= "PCOPY,";
    if ($_my_post['admin_right_access_PDELETE'] == true) $admin_right_access .= "PDELETE,";
    return $admin_right_access;
  }

  function get_admin_cat_access($_my_post) {
    $n = 0;
    $admin_cat_access_fields = intval($_my_post['admin_cat_access_fields']);
    while ($n < $admin_cat_access_fields) {
      if ($_my_post['admin_cat_access_' . $n] == true) {
        $admin_cat_access .= $_my_post['admin_cat_access_' . $n] . ",";
      }
      $n = $n + 1;
    }
...
?>
              <tr class="dataTableRowBoxes">
                <td colspan=2 class="dataTableContent" valign="top" align="right"><?php if ($_GET['gPath'] != 1) { echo  '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS, 'gID=' . $_GET['gPath']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_save.gif', IMAGE_SAVE); } else { echo tep_image_submit('button_back.gif', IMAGE_BACK); } ?>&nbsp;</td>
              </tr>
            </table></form>
<?php
 } elseif ($_GET['gID']) {
...
// Thomas Schittli: Bugfixes: Button 'Save' and not 'Insert'
      $contents[] = array('align' => 'center', 'text' => '<br>' .
                          tep_image_submit('button_save.gif', IMAGE_SAVE, 'onClick="validateForm();return document.returnValue"') .
